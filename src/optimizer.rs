...
impl Optimizations {
    ...
    pub fn and_constant_folding(self) -> Self {
        Self { constant_folding: true, ..self }
    }

    pub fn all(self) -> Self {
        self
            .and_constant_folding()
    }
}

fn constant_folding<C: CompilerContext + ?Sized>(context: &mut C, _optimizations: &Optimizations, root: C::ID) -> Result<(bool, C::ID), C::E> {
    context.for_each(
        &|_id, symbol, shared | {
            match &*symbol.name {
                "+" | "-" | "*" | "/" => {
...
pub fn optimize<C: CompilerContext + ?Sized>(context: &mut C, optimizations: &Optimizations, mut root: C::ID) -> Result<C::ID, C::E> {
    loop {
