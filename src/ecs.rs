...
impl<'source> Context<'source> {
    fn get<T: 'source>(&self, id: ID) -> Result<&T, ECSError> where Self: Provider<'source, T>{
        <Context<'source> as Provider<'source, T>>::get(self, id)
    }
    #[allow(unused)]
    fn get_mut<T: 'source>(&mut self, id: ID) -> Result<&mut T, ECSError> where Self: Provider<'source, T> {
...
macro_rules! make_provider {
    ($ctx: ty, $type: ty, $kind: tt, $accessor: tt) => {
            impl<'a> ArenaProvider<'a, $type> for $ctx {
                ...
                fn make_entity(id: ID) -> Entity {
                    Entity::$kind(ComponentID {id, ty: PhantomData })
                }
...
mod test {
    ...
    fn cannot_access_incorrect_node() {
        ...
        assert_eq!(format!("{:?}", call), format!("Err(ComponentNotFound({:?}))", hello));
    }
